...
- [x] **Step 2: `Param` / `PreparedSql`（C-1, C-2）とプレースホルダ計数**
...
- [x] **Step 2b: `ExecutedStatement`（C-7）、`LikeEscape`、`ResultSetHandler<R>`**
...
- [x] **Step 5: `BindSqlBuilder`（C-5）の実装**
...
- [x] **Step 7: `PreparedStatementBinder`（C-6）の実装**
...
- [x] **Step 8: `ExecutedStatement` 記録と `DBAccessManager`（M-5）の拡張**
...
- [x] `SQLParserTest` が 1 行も変更なしで緑（Step 4 のゲート） — 10/10 green, assertions unchanged
- [x] AC-5（FR-2.1、NUMERIC/FLOAT の検証）が `ValueRulesTest` で判定できる
- [x] AC-6（FR-2.2、BR-18 — バインド経路でシングルクォートが `''` に二重化されない）が `BindSqlBuilderTest.testQuoteIsNotDoubledOnBindPath` で判定できる（iteration 1 レビュー B-1 の是正で追加）
- [x] AC-8（FR-4.1/4.2、実行記録）が `DBAccessManagerTest` で判定できる
- [x] FR-8.1（mock からバインド位置・値を取得できる）が `MockPreparedStatementTest` で判定できる
- [x] 新規 compile 依存 0 件、Java 8 のみ（`mvn -q compile` 成功） — confirmed; full `mvn -o test` = 231/231 green（iteration 1 レビューの AC-6 是正で 1 本増加）
