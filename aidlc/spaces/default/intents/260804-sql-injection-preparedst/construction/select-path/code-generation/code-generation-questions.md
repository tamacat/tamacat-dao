...
- [ ] **Step 3: `Search.getSearchParam()`（public）の追加**
...
- [ ] **Step 4: `QueryImpl`（M-2）の WHERE 2 状態**
...
- [ ] **Step 5: `QueryImpl.getSelectPreparedSql()` の実装**
...
- [ ] **Step 6b: `andOuterJoin` の非推奨化とバインド版追加（Q2 = C）**
...
- [ ] **Step 7: `Dao` / `DaoAdapter`（M-4）の実行経路切り替え**
...
- [ ] `SearchTest` が 1 行も変更なしで緑（Step 2 のゲート）
- [ ] `QueryImplTest` が 1 行も変更なしで緑（Step 4 のゲート）
- [ ] AC-1 / AC-2 / AC-3（Step 5）、AC-4（Step 6）が mock の位置・値アサートで判定できる
- [ ] `getSelectSQL()` と `getSelectPreparedSql()` の述語数が一致する同期テストがある（BR-1/BR-3 の失敗様式を検出する唯一の手段）
- [ ] 既存 public メソッドの戻り値が不変（BR-10。`getSelectSQL()` 等の出力が変更前と一致）
