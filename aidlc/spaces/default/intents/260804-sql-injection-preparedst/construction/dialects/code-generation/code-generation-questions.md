...
- [ ] **Step 1: `OracleValueConvertFilter` の null ガード（FR-6.4）**
...
- [ ] **Step 2: `MySQLDao.searchList` の新経路化（FR-5.1 / FR-6.3）**
...
- [ ] **Step 3: `OracleDao.searchListForOracle` の修正・有効化（FR-6.1）**
...
- [ ] `OracleSearchTest` に null ケースが追加され緑（Step 1、AC-10）
- [ ] `MySQLDaoTest` が 1 行も変更なしで緑（Step 2 のゲート）
- [ ] AC-9（W-1〜W-4 の 4 状態、未バインド `?` 0 個）が `OracleDaoTest` で判定できる（Step 3 のゲート）
- [ ] 新規 public/protected メンバ 0 件（`compose`/`wrapRownum` は private static）
