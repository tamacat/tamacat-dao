...
- [ ] **Step 3: `QueryImpl` の SELECT 句への適用**
...
- [ ] **Step 4: `DataType.FUNCTION` は対象外であることの確認（不作為）**
...
- [ ] `SortTest` が 1 行も変更なしで緑（Step 2 のゲート）
- [ ] `QueryImplTest` が 1 行も変更なしで緑（Step 3 のゲート）
- [ ] `IdentifierRules.validate(null)` が例外を投げず素通しすることを確認するテストがある（BR-14）
- [ ] `getFunctionName()==null` の `Column` で例外にならない回帰テストがある（BR-14、§ 6 引き継ぎ 4）
- [ ] `getSelectSQL()` と `getSelectPreparedSql()` の両方が同じ検証を受けることを確認するテストがある（§ 6 引き継ぎ 3）
- [ ] 新規 public 型は `IdentifierRules` の 1 件のみ、新規 compile 依存 0 件、Java 8 のみ
